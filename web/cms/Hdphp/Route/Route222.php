...
class Route
{
    ...
    {
        //请求内容
        if (C('http.url_type') == 3 && isset($_GET[C("http.pathinfo_var")]))
        {
            $query = $_GET[C("http.pathinfo_var")];
        } else if (C('http.url_type') == 1 && isset($_SERVER['PATH_INFO']))
        {
        ...
        //拆分后的GET变量
        $gets = '';
        if ((C('http.url_type') == 1 && isset($_SERVER['PATH_INFO'])) || (C('http.url_type') == 3 && isset($_GET[C("http.pathinfo_var")])))
        {
            $url = str_replace(array('&', '='), C('http.pathinfo_dli'), $url);
            $args = explode(C('http.pathinfo_dli'), $url);

            //模块
            if (empty($args[0]))
            {
                $_GET[C('http.var_module')] = C('http.default_module');
            }
            else 
            {
                if ($args[0] == C('http.var_module'))
                {
                    $_GET[$args[0]] = $args[1];
                    array_shift($args);
                    array_shift($args);
            ...
            if (empty($args[0]))
            {
                $_GET[C('http.var_controller')] = C('http.default_controller');
            }
            else
            {
                if ($args[0] == C('http.var_controller')) 
                {
                    $_GET[$args[0]] = $args[1];
                    array_shift($args);
                    array_shift($args);
                ...
                {
                    $_GET[C('http.var_controller')] = $args[0];
                    array_shift($args);
                }
            }
            //动作
            if (empty($args[0]))
            {
                $_GET[C('http.var_action')] = C('http.default_action');
            }
            else
            {
                if ($args[0] == C('http.var_action'))
                {
                    $_GET[$args[0]] = $args[1];
                    array_shift($args);
                    array_shift($args);
            ...
            //模块
            if (!isset($_GET[C("http.var_module")]))
            {
                $_GET[C("http.var_module")] = C("http.default_module");
            }
            //控制器
            if (!isset($_GET[C("http.var_controller")]))
            {
                $_GET[C('http.var_controller')] = C('http.default_controller');
            }
            //动作方法
            if (!isset($_GET[C("http.var_action")]))
            {
                $_GET[C('http.var_action')] = C('http.default_action');
            }
        }
        //转模块名大小写
        $_GET[C('http.var_module')] = ucwords($_GET[C('http.var_module')]);

        //以下划线分隔的模块名称改为pascal命名如hdphp_user=>HDPhpUser
        $_GET[C('VAR_CONTROLLER')] = str_replace('!', '', ucwords(str_replace('_', '!', $_GET[C('http.var_controller')])));

        //兼容模式删除其变量
        if (C('http.url_type') == 2)
        {
            unset($_GET[C('http.pathinfo_var')]);
        }
    ...
    private function setConst()
    {
        ...
        $request_uri = trim($_SERVER['REQUEST_URI'],'/')?'/' . trim($_SERVER['REQUEST_URI'], '/'):'';
        defined('__URL__') or define("__URL__", __HOST__ .$request_uri );
        //应用URL地址
        defined('__APP__') or define("__APP__", __ROOT__ . '/' . basename(APP_PATH));
        //模块
        defined('MODULE') or define("MODULE", ucfirst($_GET[C('http.var_module')]));
        //控制器
        defined('CONTROLLER') or define("CONTROLLER", ucfirst($_GET[C('http.var_controller')]));
        //方法
        defined('ACTION') or define("ACTION", $_GET[C('http.var_action')]);
        // URL类型    1:pathinfo  2:普通模式  3:兼容模式
        switch (C("http.url_type")) {
            //普通模式
            case 2:
                defined('__MODULE__') or define("__MODULE__", __WEB__ . '?' . C('http.var_module') . '=' . MODULE);
                defined('__CONTROLLER__') or define("__CONTROLLER__", __MODULE__ . '&' . C('http.var_controller') . '=' . CONTROLLER);
                defined('__ACTION__') or define("__ACTION__", __CONTROLLER__ . '&' . C('http.var_action') . '=' . ACTION);
                break;
            //兼容模式
            case 3:
                defined('__MODULE__') or define("__MODULE__", __WEB__ . '?' . C("http.pathinfo_var") . '=/' . MODULE);
     ...
     * @return mixed
     */
    private function parseRoute($query)
    {
        $query = str_ireplace(C('http.html_suffix'), '', trim($query, '/'));
        $route = C('route');
        if (empty($route))
        {
            return $query;
     ...
     */
    public function removeUrlParam($var, $url = null)
    {
        if (is_null($url)) {
            $url = __URL__;
        }
        $url = C('http.url_type') == 2 ? $url . '&' : $url . C("PATHINFO_DLI");
        switch (C('http.url_type')) {
            case 2: //普通模式
        ...
        if (preg_match('@[^\w/]@i', $path)) {
            ...
            switch (C("http.url_type")) {
                ...
                case 3:
                    ...
                    break;
                case 2:
                    $url = C("http.var_module") . '=' . $action[C("http.var_module")] . '&' . C("http.var_controller") . '=' . $action[C("http.var_controller")] . '&' .
                        C("http.var_action") . '=' . $action[C("http.var_action")];
                    break;
            }
            /**
             * 参数$args为字符串时转数组
