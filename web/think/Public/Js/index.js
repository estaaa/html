...
function run(){
    ...
    $("#lunbo .carousel .right .bigimg a").eq(runNum).stop().css({display:"block"}).siblings().css({display:"none"});
    if(runNum==6){
      $("#lunbo .carousel .right .clickico span").eq(5).css({background:"#3a3c40"});
      $("#lunbo .carousel .right .bigimg a").eq(5).stop().css({display:"none"});
      $("#lunbo .carousel .right .clickico span").eq(0).css({background:"#B61924"});
      $("#lunbo .carousel .right .bigimg a").eq(0).stop().css({display:"block"}).siblings().css({display:"none"});
      runNum=0;
    }
}
...
function run1(){
    ...
    $("#adico .right .bigimg a").eq(runNum1).stop().css({display:"block"}).siblings().css({display:"none"});
    if(runNum1==6){
      $("#adico .right .clickico span").eq(5).css({background:"#3a3c40"});
      $("#adico .right .bigimg a").eq(5).stop().css({display:"none"});
      $("#adico .right .clickico span").eq(0).css({background:"#B61924"});
      $("#adico .right .bigimg a").eq(0).stop().css({display:"block"}).siblings().css({display:"none"});
      runNum1=0;
    }
}
...
var bgImg    = ["url('"+ROOT+"/Images/1382542488099.png')",'none',"url('"+ROOT+"/Images/1382593860805.png')",'none',"url('"+ROOT+"/Images/1421650822918.png')",'none',"url('"+ROOT+"/Images/1382542518162.png')",'none',"url('"+ROOT+"/Images/1382542488099.png')"];
...
$.each($('.jsCol'),function(x,y){
  $.each($('.jsCol').eq(x).find('.coBg'),function(xx,yy){
    ...
    // 获得随机首发 热卖 新品
     $('.jsCol').eq(x).find('.coBg').eq(xx).find('s').css({backgroundImage:bgImg[Math.floor(Math.random()*bgImg.length)]})
  })
})
